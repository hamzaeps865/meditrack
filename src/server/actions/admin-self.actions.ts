...
// ─── Update Own Profile (name only; email is immutable) ───────────────────────

export async function updateOwnAdminProfile(input: unknown) {
...
export async function changeOwnAdminPassword(input: unknown) {
  const session = await requireRole(['admin']);
