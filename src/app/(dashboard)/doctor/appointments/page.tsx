...
const statusConfig: Record<string, {
 ...
 checked_in: { dot: 'bg-amber-400',  label: 'Checked-in', badge: 'bg-amber-50 text-amber-600 border border-amber-200 px-2.5 py-0.5 text-xs font-semibold', rowHighlight: true },
 in_progress: { dot: 'bg-primary',   label: 'In-Progress', badge: 'bg-primary text-primary-foreground px-2.5 py-0.5 text-xs font-semibold', rowHighlight: true },
 completed:  { dot: 'bg-emerald-500', label: 'Completed',  badge: 'text-emerald-600 font-semibold' },
 cancelled:  { dot: 'bg-red-400',   label: 'Cancelled',  badge: 'bg-red-50 text-red-500 border border-red-200 px-2.5 py-0.5 text-xs font-semibold', dim: true },
     ...
     <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-0.5">
       <Link
        href={prevHref}
        aria-label="Previous"
        className="h-8 w-8 flex items-center justify-center border border-border
       ...
       <Link
        href={todayHref}
        className={`h-8 px-3 flex items-center justify-center border border-border
         ...
         >
          ...
          <div>
           ...
           {(appt.status === 'scheduled' || appt.status === 'completed') && (
            <div className="flex flex-col gap-0.5">
             <span className={`flex items-center gap-1.5 text-sm ${cfg.badge}`}>
              <span className={`h-2 w-2 shrink-0 ${cfg.dot}`} />
              {cfg.label}
             </span>
           ...
           {(appt.status === 'checked_in' || appt.status === 'in_progress' || appt.status === 'cancelled' || appt.status === 'no_show') && (
            <span className={`inline-flex items-center gap-1.5 ${cfg.badge}`}>
             <span className={`h-1.5 w-1.5 shrink-0 ${cfg.dot}`} />
             {cfg.label}
            </span>
           )}
          </div>
          ...
          <div className="flex justify-end">
           ...
           {isInProgress && (
            <Link
             href={`/doctor/appointments/${appt.id}`}
             className="h-9 px-4 bg-foreground text-white text-sm
