     ...
     <div className="bg-white border border-border p-5 shadow-sm">
      ...
      <div className="flex items-center gap-6">
       <div className="relative h-32 w-32 shrink-0">
        <div className="absolute inset-0 " style={{ background: donutGradient }} />
        <div className="absolute inset-4 bg-white flex flex-col items-center justify-center">
        ...
        {data.rating.distribution.map((d) => (
         <div key={d.star} className="flex items-center gap-2">
          ...
          <div className="flex-1 h-2.5 bg-muted overflow-hidden">
           <div className="h-full bg-amber-400 " style={{ width: `${totalRatings > 0 ? (d.count / totalRatings) * 100 : 0}%` }} />
          </div>
       ...
       <div className="space-y-3">
        {data.topDiagnoses.map((d, i) => (
         <div key={d.diagnosis} className="flex items-center gap-3">
          <span className="h-6 w-6 bg-primary/10 text-primary flex items-center justify-center text-xs font-bold shrink-0">
           {i + 1}
          </span>
          <div className="flex-1 min-w-0">
           ...
           <div className="h-2 bg-muted overflow-hidden">
            <div className="h-full bg-primary " style={{ width: `${(d.count / maxDiagnosis) * 100}%` }} />
           </div>
          </div>
         </div>
        ))}
...
function Legend({ color, label, value, pct }: { color: string; label: string; value: number; pct: number }) {
 return (
  <div className="flex items-center gap-2">
   <span className={`h-2.5 w-2.5 ${color} shrink-0`} />
