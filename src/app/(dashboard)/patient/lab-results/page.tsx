       ...
       return (
        <div key={order.id} className="premium-card premium-card-pad">
         <div className="flex items-start justify-between mb-3">
          ...
          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 ${cfg.bg} ${cfg.color}`}>
           {order.status}
          </span>
         </div>
         ...
         <div className="flex flex-wrap gap-2 mt-3">
          <Link href={`/patient/lab-results/${order.id}`} className="inline-flex items-center gap-1.5 border border-border px-3 py-2 text-xs font-medium text-foreground hover:bg-muted/50 transition-colors">
           <FileText className="h-3.5 w-3.5" /> View Report
          </Link>
          <Link href={`/patient/lab-results/${order.id}/print`} target="_blank" className="inline-flex items-center gap-1.5 bg-primary px-3 py-2 text-xs font-medium text-white hover:opacity-90 transition-opacity">
