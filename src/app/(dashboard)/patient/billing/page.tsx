    ...
    <div className="min-h-full bg-[#f5f7fa]">
      ...
      <div className="px-6 py-8 max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-white rounded-2xl border border-border p-5 shadow-sm">
            <div className={`h-10 w-10 rounded-xl flex items-center justify-center mb-3 ${totalOutstanding > 0 ? 'bg-amber-50' : 'bg-muted'}`}>
                ...
                {patientInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-muted/20">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${statusStyles[inv.status] ?? statusStyles.pending}`}>
                        {inv.status}
                      </span>
                    </td>
