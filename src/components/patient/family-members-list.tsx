  ...
  return (
    <>
      ...
      <button
        ...
        className="w-full mb-4 h-12 rounded-2xl border-2 border-dashed border-border
            ...
            return (
              <div key={m.id} className="bg-white rounded-2xl border border-border p-5 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-1"><button type="button" onClick={() => setEditing(m)} className="h-8 w-8 flex items-center justify-center rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/5" aria-label="View or edit"><Pencil className="h-3.5 w-3.5" /></button><button
                      ...
                      className="h-8 w-8 flex items-center justify-center rounded-lg text-muted-foreground hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50"
                      aria-label="Remove"
                    >
                ...
                <div className="flex flex-wrap gap-1.5">
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full capitalize ${genderColor[m.gender] ?? 'bg-muted text-muted-foreground'}`}>
                    {m.gender}
                  </span>
                  {m.bloodGroup && (
                  ...
                  {m.city && (
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-muted text-muted-foreground flex items-center gap-0.5">
