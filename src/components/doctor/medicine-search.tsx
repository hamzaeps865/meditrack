  ...
  return (
    <div ref={containerRef} className="relative">
      <input
        ...
        disabled={disabled}
        className="w-full h-8 px-2.5 rounded-md border border-border bg-muted/30 text-sm text-foreground
      ...
      {showDropdown && results.length > 0 && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white rounded-lg border border-border shadow-lg max-h-60 overflow-y-auto">
          {results.map((r) => (
            <button
              key={r.id}
              type="button"
            ...
            >
              ...
              {r.category && (
                <span className="inline-block mt-0.5 text-[10px] px-1.5 py-0.5 rounded-full bg-primary/10 text-primary">{r.category}</span>
              )}
            </button>
          ))}
        </div>
