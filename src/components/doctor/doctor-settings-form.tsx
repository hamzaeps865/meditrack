...
function Toggle({ defaultOn = true, label }: { defaultOn?: boolean; label: string }) {
  ...
  return (
    ...
    >
      <div className={`h-4 w-8 rounded-full flex items-center px-0.5 transition-colors ${on ? 'bg-primary' : 'bg-muted'}`}>
        <div className={`h-3 w-3 rounded-full bg-white shadow-sm transition-all ${on ? 'ml-auto' : ''}`} />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 ml-0 sm:ml-12">
          <div>
            ...
            <input
              ...
              className="w-full h-10 px-3 rounded-lg border border-border bg-muted/30 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:bg-white transition-colors"
            />
          </div>
          <div>
            ...
            <input
              ...
              value={initialEmail}
              disabled
              className="w-full h-10 px-3 rounded-lg border border-border bg-muted/50 text-sm text-muted-foreground cursor-not-allowed"
            />
          </div>
        </div>
        ...
        <div className="flex justify-end mt-4">
          <button
            ...
            disabled={isPending}
            className="h-9 px-5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
          >
        ...
        <form onSubmit={handleChangePassword} className="grid grid-cols-1 sm:grid-cols-3 gap-4 ml-0 sm:ml-12">
          ...
          <div className="sm:col-span-3 flex justify-end">
            <button
              type="submit"
              disabled={isPending}
              className="h-9 px-5 rounded-lg border border-border bg-white text-sm font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {isPending && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
              Update Password
            </button>
