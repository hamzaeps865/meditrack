  ...
  <>
   ...
   {!open && (
    ...
    >
     ...
     {/* Pulse ring */}
     <span className="absolute inset-0 bg-emerald-400 animate-ping opacity-20" />
     {/* Badge */}
     <span className="absolute -top-1 -right-1 h-5 w-5 bg-emerald-500
    ...
    <div className="fixed bottom-6 right-6 z-50 w-[calc(100vw-3rem)] sm:w-96 h-[500px] max-h-[calc(100vh-3rem)]
     ...
     <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 px-4 py-3 flex items-center justify-between">
      ...
      <button
       ...
       className="h-8 w-8 flex items-center justify-center text-white/80 hover:bg-white/20 transition-colors"
      >
       <X className="h-4 w-4" />
      </button>
     </div>
     ...
     <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3 bg-emerald-50/30">
      ...
      {loading && (
       <div className="flex gap-2">
        <div className="h-7 w-7 bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0">
         <Sparkles className="h-3.5 w-3.5" />
        </div>
        <div className="bg-white border border-border  px-3.5 py-2.5">
         <div className="flex items-center gap-1">
          <span className="h-1.5 w-1.5 bg-emerald-400 animate-bounce" style={{ animationDelay: '0ms' }} />
          <span className="h-1.5 w-1.5 bg-emerald-400 animate-bounce" style={{ animationDelay: '150ms' }} />
          <span className="h-1.5 w-1.5 bg-emerald-400 animate-bounce" style={{ animationDelay: '300ms' }} />
         </div>
        </div>
       </div>
      )}
        ...
        <div className="flex flex-wrap gap-1.5">
         {SUGGESTIONS.map((s) => (
          <button
           ...
           className="px-2.5 py-1 border border-border bg-white text-[11px] font-medium text-foreground hover:bg-emerald-50 hover:border-emerald-300 transition-all"
          >
           {s}
          </button>
         ))}
     ...
     <div className="border-t border-border px-3 py-2.5 bg-white">
      <div className="flex items-end gap-2">
       <textarea
        ...
        rows={1}
        className="flex-1 px-3 py-2 border border-border bg-emerald-50/30 text-xs text-foreground placeholder:text-muted-foreground/60 resize-none focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
       ...
       <button
        ...
        className="h-9 w-9 bg-emerald-600 text-white flex items-center justify-center hover:bg-emerald-700 disabled:opacity-50 shrink-0 transition-colors"
       >
