 ...
 return (
  <div>
   <div className="flex items-center justify-between mb-4">
    ...
    <button type="button" onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 h-8 px-3 bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90">
     ...
     <div className="bg-white shadow-xl border border-border w-full max-w-md max-h-[90vh] overflow-y-auto">
      ...
      <form onSubmit={handleAddStock} className="p-5 space-y-4">
       ...
       <div className="flex justify-end gap-2 pt-2">
        <button type="button" onClick={() => setShowAdd(false)} className="h-10 px-4 border border-border text-sm font-medium text-foreground hover:bg-muted">Cancel</button>
        <button type="submit" disabled={isPending} className="h-10 px-5 bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 disabled:opacity-60 flex items-center gap-2">
    ...
    {filtered.map((m) => (
     <div key={m.id} className="bg-white border border-border p-4 shadow-sm">
      <div className="flex items-start justify-between">
       ...
       {m.form && <span className="text-[10px] font-medium px-1.5 py-0.5 bg-muted text-muted-foreground shrink-0">{m.form}</span>}
      </div>
      <div className="flex flex-wrap gap-1.5 mt-2">
       {m.category && <span className="text-[10px] px-1.5 py-0.5 bg-primary/10 text-primary">{m.category}</span>}
       {m.strength && <span className="text-[10px] px-1.5 py-0.5 bg-muted text-muted-foreground">{m.strength}</span>}
      </div>
     ...
     {pending.map((item) => (
      <div key={item.id} className="bg-white border border-border p-4 shadow-sm">
       ...
       <div className="flex items-end gap-2">
        ...
        <button type="button" onClick={() => handleDispense(item)} disabled={isPending} className="h-10 px-4 bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 disabled:opacity-60 flex items-center gap-1.5">
